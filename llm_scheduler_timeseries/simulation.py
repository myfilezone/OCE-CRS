"""Simulation orchestration for the LLM scheduler timeseries experiments."""
...
import llm_scheduler_timeseries as cc
from llm_scheduler_core.request_model import Request
from llm_scheduler_timeseries import (
    BertEmbedder,
    MetricsCollector,
...
def run_simulation(scheduler_instance, embedder_instance, processed_dataset_df):
    ...
    # Get final metrics, time-series log, and completed requests data
    # FIX: Unpack 3 values instead of 2
    summary_metrics, timeseries_log, completed_requests_df = metrics_coll.get_final_metrics_and_timeseries(cc.current_time)
...
def run_experiment(experiment_config_override={}, epochs=1, save_results=True):
    ...
    for epoch in range(epochs):
        ...
        # Deep copy default config and apply overrides for THIS epoch
        ...
        for sch_name, sch_fact_fn in schedulers_to_run.items():
            ...
            try:
                ...
                # Run the simulation
                summary_data_this_run, timeseries_log_this_run, completed_req_df_this_run = run_simulation(sch_inst, embedder_instance, processed_dataset)
                ...
                if timeseries_log_this_run: # Check if the list is not empty
                    for rec in timeseries_log_this_run:
                        rec_c = rec.copy() # Copy the record
                        rec_c['epoch'] = epoch # Add epoch number
                        rec_c['scheduler_name'] = sch_name # Add scheduler name
                        all_epochs_timeseries_records_list.append(rec_c) # Append to the flat list
                    logging.info(f"Collected {len(timeseries_log_this_run)} timeseries data points for Epoch {epoch+1}, {sch_name}")
                ...
                if isinstance(completed_req_df_this_run, pd.DataFrame) and not completed_req_df_this_run.empty:
                    completed_df_copy = completed_req_df_this_run.copy() # Work on a copy
                    completed_df_copy['epoch'] = epoch # Add epoch column
                    completed_df_copy['scheduler_name'] = sch_name # Add scheduler name column
                    all_epochs_completed_requests_dfs_list.append(completed_df_copy) # Append the DataFrame
    ...
    if save_results:
        try:
            ...
            if all_epochs_sum_list:
                ...
                if 'epoch' in cols: cols.remove('epoch')
                if 'scheduler_name' in cols: cols.remove('scheduler_name')
                ...
                summary_filename = f"exp_summary_batch_addbaseline_{cc.CONFIG.get('llm_name', 'unknown_llm')}_epoch_{cc.CONFIG.get('cnducb_nu', '')}nu_{cc.CONFIG.get('clinucb_lambda_reg', '')}lam.csv"
                sum_df.to_csv(summary_filename, index=False)
                logging.info(f"Summary metrics saved to {summary_filename}")
                print(f"\nSummary results saved: {summary_filename}")
                print("\nSummary Results Table (first few rows):")
                print(sum_df.head().to_string())
                if len(sum_df) > 5: print("...")
            ...
            if all_epochs_timeseries_records_list:
                ...
                timeseries_filename = f"exp_timeseries_batch_addbaseline_{cc.CONFIG.get('llm_name', 'unknown_llm')}_epoch_{cc.CONFIG.get('cnducb_nu', '')}nu_{cc.CONFIG.get('clinucb_lambda_reg', '')}lam.csv"
                ts_df.to_csv(timeseries_filename, index=False)
                logging.info(f"Timeseries data saved to {timeseries_filename}")
                print(f"Time-series results saved: {timeseries_filename}")
            else:
                logging.warning("No timeseries data collected.")
                print("\nNo time-series data was collected.")
            ...
            if all_epochs_completed_requests_dfs_list:
                ...
                key_cols = ['epoch', 'scheduler_name', 'id', 'arrival_time', 'start_time', 'completion_time']
                for col in key_cols:
                    if col in cols: cols.remove(col)
                cols = key_cols + cols
                ...
                if not float_cols_completed_to_round.empty:
                    all_completed_df[float_cols_completed_to_round] = all_completed_df[float_cols_completed_to_round].round(4)

                completed_filename = f"exp_completed_requests_batch_addbaseline_{cc.CONFIG.get('llm_name', 'unknown_llm')}_epoch_{cc.CONFIG.get('cnducb_nu', '')}nu_{cc.CONFIG.get('clinucb_lambda_reg', '')}lam.csv"
                all_completed_df.to_csv(completed_filename, index=False)
                logging.info(f"Completed requests data saved to {completed_filename}")
    ...
    final_ts_df = pd.DataFrame(all_epochs_timeseries_records_list) if all_epochs_timeseries_records_list else pd.DataFrame()
    final_completed_df_return = pd.concat(all_epochs_completed_requests_dfs_list, ignore_index=True) if all_epochs_completed_requests_dfs_list else pd.DataFrame()
...
if __name__ == "__main__":
    ...
    if not results_timeseries_df_main.empty:
        ...
        print("Timeseries DataFrame is empty.")
